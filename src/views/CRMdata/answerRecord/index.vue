    ...
    <div class="tableRow">
      ...
      <div>
        ...
        >
          ...
          <el-table-column property="reviewnum" label="回访台数" align="center"></el-table-column>
      ...
      <div class="tableBottom">
        <div>
          ...
          <el-button @click="reverseSelect(huifangList)" type="primary" size="mini">反选</el-button>
...
// 查询回访记录
import { queryHuiFangData } from "@/api/CRMdata";
...
export default {
  ...
  methods: {
    ...
    fetchListData() {
      ...
      queryHuiFangData(data).then(res => {
        this.huifangList = res.data.list;
    ...
    // 全选的方法
    allSelect() {
      this.$refs.huifangList.toggleAllSelection();
    },
    // 取消选择的方法
    cancelSelect() {
      this.$refs.huifangList.clearSelection();
    },
    // 反选的方法
    reverseSelect(rows) {
      if (rows) {
        rows.forEach(row => {
          this.$refs.huifangList.toggleRowSelection(row);
        });
      }
    },
    // 批量导出
    excelBtn() {
      ...
      if (this.multipleSelection.length == 0) {
        ...
        queryHuiFangData(this.pageQuery).then(res => {
          ...
          XLSX.utils.book_append_sheet(wb, ws, "回访记录列表");
          XLSX.writeFile(wb, "回访记录列表.xlsx");
        });
      } else {
