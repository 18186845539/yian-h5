          ...
          <div class="searchRow">
            ...
            <span>
              ...
              <el-button
                ...
                size="small"
              >更多</el-button>
              <el-button type="primary" icon="el-icon-search" size="small" @click="excelBtn()">批量导出</el-button>
    ...
    <div class="tableRow">
      ...
      <div>
        ...
        >
          ...
          <el-table-column property="specialNum" label="专项台数" align="center"></el-table-column>

          <el-table-column property="shuttleDate" label="专线日期" align="center"></el-table-column>
      ...
      <div class="tableBottom">
        <div>
          ...
          <el-button @click="reverseSelect(zhuanxiangList)" type="primary" size="mini">反选</el-button>
...
// 查询故障与配件-列表统计数据
import { queryZhuanXiangData } from "@/api/CRMdata";
...
export default {
  ...
  methods: {
    ...
    fetchListData() {
      ...
      queryZhuanXiangData(data).then(res => {
        this.zhuanxiangList = res.data.list;
    ...
    // 全选的方法
    allSelect() {
      this.$refs.zhuanxiangList.toggleAllSelection();
    },
    // 取消选择的方法
    cancelSelect() {
      this.$refs.zhuanxiangList.clearSelection();
    },
    // 反选的方法
    reverseSelect(rows) {
      if (rows) {
        rows.forEach(row => {
          this.$refs.zhuanxiangList.toggleRowSelection(row);
        });
      }
    },
    // 批量导出
    excelBtn() {
      ...
      if (this.multipleSelection.length == 0) {
        ...
        queryZhuanXiangData(this.pageQuery).then(res => {
          ...
          res.data.list.map(item => {
            ...
            arr.push(item.specialNum);
            arr.push(item.shuttleDate);
            arr.push(item.note);
            arr.push(item.owner);
          ...
          XLSX.utils.book_append_sheet(wb, ws, "专项记录列表");
          XLSX.writeFile(wb, "专项记录列表.xlsx");
        });
      } else {
