  ...
  <div class="pageContainer" ref="pageContainer">
    ...
    <div class="tableRow">
      ...
      <div class="tableTitle">
        ...
        <span>
          <el-button type="success" icon="el-icon-plus" size="small" @click="toModifyPage()">添加员工工单</el-button>
        </span>
      </div>
      ...
      <div>
        <el-table
          ...
          :data="staffRepairsList"
          ref="staffRepairsList"
        ...
        >
          ...
          <el-table-column property="status" label="状态" align="center">
            <template slot-scope="scope">
              ...
              <el-tag
                ...
                type="success"
                size="small"
              >{{scope.row.status==0? "待派单":""}}</el-tag>
              <!-- 待派单 -->
              <el-tag
                v-if="scope.row.status==1"
                type="success"
                size="small"
              >{{scope.row.status==1? "待派单":"" }}</el-tag>
              <!-- 已拒绝 -->
              <el-tag
                v-if="scope.row.status==2"
                type="warning"
                size="small"
              >{{scope.row.status==2? "已拒绝 ":""}}</el-tag>
              <!-- 再次派单 -->
              <el-tag
          ...
          <el-table-column property="date" label="审核且派单" width="120" align="center">
            <template slot-scope="scope">
              <el-popover trigger="hover" placement="top">
                <p>只有未审核、待派单、已拒绝状态才可以审核且派单哦！</p>
                <div slot="reference" class="name-wrapper">
                  <el-button
                    v-if="scope.row.status == 3|| scope.row.status == 4|| scope.row.status == 5|| scope.row.status == 6|| scope.row.status == 7|| scope.row.status == 8"
                    type="info"
                    disabled
                    size="mini"
                  >派单</el-button>
              ...
              <el-button v-if="scope.row.status == 0" @click="auditPage(scope.row)" size="mini">派单</el-button>
      ...
      <div class="tableBottom">
        <div>
          ...
          <el-button @click="reverseSelect(staffRepairsList)" type="primary" size="mini">反选</el-button>
  ...
  methods: {
    ...
    fetchListData() {
      ...
      queryApplyJobGroupList(data).then(res => {
        this.staffRepairsList = res.data.list;
    ...
    // 全选的方法
    allSelect() {
      this.$refs.staffRepairsList.toggleAllSelection();
    },
    // 取消选择的方法
    cancelSelect() {
      this.$refs.staffRepairsList.clearSelection();
    },
    // 反选的方法
    reverseSelect(rows) {
      if (rows) {
        rows.forEach(row => {
          this.$refs.staffRepairsList.toggleRowSelection(row);
        });
      }
    },
    // 单独删除派工单
    ...
    excelBtn() {
      ...
      if (this.multipleSelection.length == 0) {
        ...
        queryApplyJobGroupList(this.pageQuery).then(res => {
          ...
          XLSX.utils.book_append_sheet(wb, ws, "员工报修单列表");
          XLSX.writeFile(wb, "员工报修单列表.xlsx");
        });
      } else {
